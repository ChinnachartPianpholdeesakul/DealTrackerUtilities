...
CREATE OR REPLACE PACKAGE C##RTR_DT8N.DTANALYSER_PCK AS
...
CREATE OR REPLACE PACKAGE BODY C##RTR_DT8N.DTANALYSER_PCK AS
...
BEGIN
  SELECT RUNNINGVALUE,MAXVALUE INTO NVALUE,NMAXVALUE FROM C##RTR_DT8N.TBRUNNINGNUMBER WHERE KEYNAME = STRKEYNAME FOR UPDATE OF RUNNINGVALUE;
  
  IF((NVALUE + 1) > NMAXVALUE)
  THEN
    UPDATE C##RTR_DT8N.TBRUNNINGNUMBER SET RUNNINGVALUE = MINVALUE WHERE KEYNAME = STRKEYNAME;
  ELSE
    UPDATE C##RTR_DT8N.TBRUNNINGNUMBER SET RUNNINGVALUE = RUNNINGVALUE + 1 WHERE KEYNAME = STRKEYNAME;
  END IF;
...
IS
BEGIN
	INSERT INTO C##RTR_DT8N.TICKETS 
		(
			 DealId
			, Processed
			, ProcessedBy
...
BEGIN
	...
	IF nCreationType = 1 THEN
		...
		IF iOldTicketsCount = 0 THEN
			...
			C##RTR_DT8N.DTANALYSER_PCK.SP_INSERTGENTICKETBYDEALTYPE( nConv_ID, nCreationType, nSetChecked1, nSetChecked2, sUserName
...
CREATE OR REPLACE FUNCTION C##RTR_DT8N.FN_GETNEXTDAY ( DayOfWeek VARCHAR2, hour_time INTEGER, min_time INTEGER)
RETURN DATE IS
BEGIN
